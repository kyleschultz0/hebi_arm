...
int SSS = 10; // SSS pins in order of encoder connection
...
void loop() 
{
  ...
  byte serial_reading;
    ...
    if (timeoutCounter < timoutLimit) //rd_pos echo received
    {
      ...
      currentPosition = (SPIWrite(nop,SSS)& 0x0F) << 8;
    
      //OR the next byte with the current position
      currentPosition |= SPIWrite(nop,SSS);
